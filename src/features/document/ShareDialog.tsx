...
export default function ShareDialog({
  ...
  async function grant(e: React.FormEvent) {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) {
        const message = data.error ?? "Could not share."
        setError(message)
        toast.error(message)
        return
      }
      const shared = email.trim()
      setEmail("")
      await load()
      toast.success(`Shared with ${shared} as ${role}`)
    } finally {
      setBusy(false)
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      {/* Wider than the default: this dialog carries full share URLs. */}
      <DialogContent className="sm:max-w-lg">
        ...
        <div>
          ...
          <ul className="space-y-1.5">
            <li className="bg-muted flex items-center justify-between gap-2 rounded-lg px-3 py-2 text-sm">
              <span className="min-w-0 truncate">
                {currentUser.display_name} <span className="text-muted-foreground">({currentUser.email})</span>
              </span>
              <span className="text-muted-foreground shrink-0 text-xs font-medium">Owner</span>
            </li>
            {shares.map((s) => (
              <li key={s.user_id} className="flex items-center justify-between gap-2 rounded-lg px-3 py-2 text-sm">
                <span className="min-w-0 truncate">
