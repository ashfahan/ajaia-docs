...
export default function Editor({
  ...
  // "Mid-edit" = focused in the editor, or a debounced save still pending.
  const isEditingRef = useRef(false)
  // Set below, once refreshContent exists; keeps the subscription off the
  // callback's identity so it never resubscribes mid-session.
  const refreshContentRef = useRef<() => Promise<void>>(async () => {})

  // Live updates: broadcast a ping after we save. When a collaborator's edit
  // arrives we pull it in automatically -- but only while this user is not
  // mid-edit. Swapping the document out from under someone who is typing would
  // lose their caret and their unsaved keystrokes, so in that case we fall back
  // to the banner and let them choose the moment.
  const { broadcastUpdate } = useDocRealtime({
    ...
    onRemoteUpdate: () => {
      ...
      void refreshContentRef.current()
    },
  })
  ...
  const editor = useEditor({
    ...
    onBlur: () => {
      // Only idle once nothing is queued to save.
      if (!saveTimer.current) isEditingRef.current = false
    },
    onUpdate: ({ editor }) => {
      if (!canEdit) return
      isEditingRef.current = true
      ...
      saveTimer.current = setTimeout(() => {
        ...
        // Stay "editing" briefly after the save so a collaborator's ping that
        // races our own write does not yank content mid-keystroke.
        setTimeout(() => {
          if (!editor.isFocused) isEditingRef.current = false
        }, 1500)
      }, 800)
    },
  })
  ...
  const refreshContent = useCallback(async () => {
    ...
    toast.success("Updated with a collaborator's changes")
  }, [doc.id, editor])

  // Keep the ref pointing at the live callback so the Realtime subscription can
  // auto-apply without depending on this callback's identity.
    ...
    <div className="min-h-screen">
      ...
      <div className="sticky top-0 z-10 border-b border-zinc-200 bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-5xl items-center gap-3 px-6 py-3">
          <Link href="/dashboard" className="shrink-0 text-sm whitespace-nowrap text-zinc-500 hover:text-zinc-800">
            &larr; Docs
          </Link>
          <input
            ...
            className="min-w-0 flex-1 rounded px-2 py-1 text-base font-semibold text-zinc-900 outline-none focus:bg-zinc-100 disabled:bg-transparent"
          />
...
function Toolbar({ editor }: { editor: TiptapEditor }) {
  ...
  return (
    <div className="mx-auto flex max-w-5xl flex-wrap items-center gap-1 px-6 pb-2">
      <Btn
        title="Bold"
        label="B"
