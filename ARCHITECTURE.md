...
After a save, the client broadcasts a ping on a `doc:<id>` channel via Supabase Realtime (`src/services/realtimeClient.ts`, `src/features/document/useDocRealtime.ts`). Other clients viewing the same document **apply the change automatically**, refetching through the authenticated API - unless that user is mid-edit (focused in the editor, or a debounced save still pending), in which case they get an "updated by a collaborator - Load latest" banner instead. That guard is the whole point: swapping the document out from under someone who is typing would cost them their caret and their unsaved keystrokes, so the automatic path only runs when it is safe and the manual path covers the rest.

This is still **live-update propagation, not** multi-cursor co-editing: the unit of sync is a whole document refetch, not a character-level merge, and simultaneous typing remains last-write-wins. Low-risk because no document state travels over the channel - only a refetch signal - so authorization and persistence stay entirely in the existing authenticated API.
